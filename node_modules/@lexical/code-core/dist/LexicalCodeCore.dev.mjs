...
function $getStartOfCodeInLine(anchor, offset) {
  ...
  while (true) {
    if (nodeOffset === 0) {
      ...
      if (!($isCodeHighlightNode(prevSibling) || $isTabNode(prevSibling) || $isLineBreakNode(prevSibling))) {
...
const VscodeLineRunRule = {
  ...
  match: /* @__PURE__ */sel.tag('div', 'br'),
  name: '@lexical/code/vscode-line-run'
};
const VscodeCodePasteOverlay = /* @__PURE__ */defineOverlayRules([VscodeWrapperRule, VscodeLineRunRule]);
...
const GitHubCodeTableRule = {
  ...
  match: /* @__PURE__ */sel.tag('table').classAll('js-file-line-container'),
...
const GitHubCodeCellByClassRule = {
  $import: (ctx, el) => ctx.$importChildren(el),
  match: /* @__PURE__ */sel.tag('td').classAll('js-file-line'),
...
function $handleShiftLines(type, event) {
  ...
  const maybeInsertionPoint = !adjacentLineIsBlank && ($isCodeHighlightNode(sibling) || $isTabNode(sibling)) ? arrowIsUp ? $getFirstCodeNodeOfLine(sibling) : $getLastCodeNodeOfLine(sibling) : null;
...
function registerCodeIndentation(editor, tabSize, escapeWithArrows) {
  ...
  ...(escapeWithArrows ? [editor.registerCommand(KEY_ARROW_DOWN_COMMAND, event => event.altKey ? false : $onEscapeDown($isCodeNode, event), COMMAND_PRIORITY_LOW), editor.registerCommand(KEY_ARROW_RIGHT_COMMAND, event => $onEscapeDown($isCodeNode, event), COMMAND_PRIORITY_LOW), editor.registerCommand(KEY_ARROW_UP_COMMAND, event => event.altKey ? false : $onEscapeUp($isCodeNode, event), COMMAND_PRIORITY_LOW), editor.registerCommand(KEY_ARROW_LEFT_COMMAND, event => $onEscapeUp($isCodeNode, event), COMMAND_PRIORITY_LOW)] : []), editor.registerCommand(KEY_TAB_COMMAND, event => {
