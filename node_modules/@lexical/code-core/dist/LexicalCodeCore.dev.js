...
function $getStartOfCodeInLine(anchor, offset) {
  ...
  while (true) {
    if (nodeOffset === 0) {
      ...
      if (!($isCodeHighlightNode(prevSibling) || lexical.$isTabNode(prevSibling) || lexical.$isLineBreakNode(prevSibling))) {
...
function $outdentLeadingSpaces(node, tabSize, selection) {
  ...
  if (oldAnchorOffset !== null) {
    selection.anchor.set(lineKey, Math.max(0, oldAnchorOffset - stripCount), 'text');
  }
  if (oldFocusOffset !== null) {
    selection.focus.set(lineKey, Math.max(0, oldFocusOffset - stripCount), 'text');
  }
  return true;
}
...
class CodeNode extends lexical.ElementNode {
  ...
  // View
  createDOM(config) {
    const element = lexical.$getDocument().createElement('code');
    ...
    if (theme) {
      element.setAttribute(THEME_DATA_ATTRIBUTE$1, theme);
    }
  ...
  updateDOM(prevNode, dom, config) {
    ...
    if (theme) {
      if (theme !== prevTheme) {
        dom.setAttribute(THEME_DATA_ATTRIBUTE$1, theme);
      }
    } else if (prevTheme) {
      dom.removeAttribute(THEME_DATA_ATTRIBUTE$1);
    }
  ...
  exportDOM(editor) {
    const element = lexical.$getDocument().createElement('pre');
...
function $convertPreElement(domNode) {
  ...
  const theme = domNode.getAttribute(THEME_DATA_ATTRIBUTE$1);
  return {
    node: $createCodeNode(language, theme)
  };
}
 ...
 * `<td>` pastes.
 */
const GitHubCodeTableOverlayRules = /* @__PURE__ */html.defineOverlayRules([{
  $import: (ctx, el) => ctx.$importChildren(el),
  match: /* @__PURE__ */html.sel.tag('tr', 'td'),
  name: '@lexical/code/github-code-table/unwrap'
}]);
const PreRule = {
  $import: (ctx, el) => [$createCodeNode(el.getAttribute(LANGUAGE_DATA_ATTRIBUTE), el.getAttribute(THEME_DATA_ATTRIBUTE)).splice(0, 0, ctx.$importChildren(el))],
  match: /* @__PURE__ */html.sel.tag('pre'),
...
const MultilineCodeRule = {
  $import: (ctx, el, $next) => {
    ...
    return [$createCodeNode(el.getAttribute(LANGUAGE_DATA_ATTRIBUTE), el.getAttribute(THEME_DATA_ATTRIBUTE)).splice(0, 0, ctx.$importChildren(el))];
  },
  match: /* @__PURE__ */html.sel.tag('code'),
...
const VscodeWrapperRule = {
  ...
  match: /* @__PURE__ */html.sel.tag('div'),
...
const VscodeLineRunRule = {
  ...
  match: /* @__PURE__ */html.sel.tag('div', 'br'),
  name: '@lexical/code/vscode-line-run'
};
const VscodeCodePasteOverlay = /* @__PURE__ */html.defineOverlayRules([VscodeWrapperRule, VscodeLineRunRule]);
 ...
 */
const GitHubCodeCellByClassRule = {
  $import: (ctx, el) => ctx.$importChildren(el),
  match: /* @__PURE__ */html.sel.tag('td').classAll('js-file-line'),
 ...
 */
function $outdentLineAtCaret(selection, tabSize) {
  const anchorNode = selection.anchor.getNode();
  // An element point (e.g. the caret on a blank line) has no line to outdent.
  if (!$isCodeHighlightNode(anchorNode) && !lexical.$isTabNode(anchorNode)) {
    return;
  }
  const firstOfLine = $getFirstCodeNodeOfLine(anchorNode);
  if (lexical.$isTabNode(firstOfLine)) {
    firstOfLine.remove();
...
function $handleShiftLines(type, event) {
  ...
  const adjacentLineIsBlank = lexical.$isLineBreakNode(sibling);
  const maybeInsertionPoint = !adjacentLineIsBlank && ($isCodeHighlightNode(sibling) || lexical.$isTabNode(sibling)) ? arrowIsUp ? $getFirstCodeNodeOfLine(sibling) : $getLastCodeNodeOfLine(sibling) : null;
  ...
  } else if (type === lexical.KEY_ARROW_UP_COMMAND) {
...
function registerCodeIndentation(editor, tabSize, escapeWithArrows) {
  ...
  ...(escapeWithArrows ? [editor.registerCommand(lexical.KEY_ARROW_DOWN_COMMAND, event => event.altKey ? false : utils.$onEscapeDown($isCodeNode, event), lexical.COMMAND_PRIORITY_LOW), editor.registerCommand(lexical.KEY_ARROW_RIGHT_COMMAND, event => utils.$onEscapeDown($isCodeNode, event), lexical.COMMAND_PRIORITY_LOW), editor.registerCommand(lexical.KEY_ARROW_UP_COMMAND, event => event.altKey ? false : utils.$onEscapeUp($isCodeNode, event), lexical.COMMAND_PRIORITY_LOW), editor.registerCommand(lexical.KEY_ARROW_LEFT_COMMAND, event => utils.$onEscapeUp($isCodeNode, event), lexical.COMMAND_PRIORITY_LOW)] : []), editor.registerCommand(lexical.KEY_TAB_COMMAND, event => {
    ...
    return true;
  }, lexical.COMMAND_PRIORITY_LOW), editor.registerCommand(lexical.INDENT_CONTENT_COMMAND, () => $handleMultilineIndent(lexical.INDENT_CONTENT_COMMAND), lexical.COMMAND_PRIORITY_LOW), editor.registerCommand(lexical.OUTDENT_CONTENT_COMMAND, () => $handleMultilineIndent(lexical.OUTDENT_CONTENT_COMMAND, tabSize), lexical.COMMAND_PRIORITY_LOW), editor.registerCommand(lexical.KEY_ARROW_UP_COMMAND, event => {
    const selection = lexical.$getSelection();
    if (!lexical.$isRangeSelection(selection)) {
      return false;
    }
...
exports.registerCodeIndentation = registerCodeIndentation;
