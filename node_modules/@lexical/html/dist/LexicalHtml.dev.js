 ...
 * @experimental
 */
const $withRenderContext = /* @__PURE__ */$withContext(DOMRenderContextSymbol, getDefaultRenderContext);
...
const DOMRenderExtension = {
  ...
  html: {
    ...
    export: new Map([[lexical.RootNode, () => {
      const element = lexical.$getDocument().createElement('div');
 ...
 * @experimental
 */
const $withImportContext = /* @__PURE__ */$withContext(DOMImportContextSymbol, getDefaultImportContext);
 ...
 */
const FORMAT_BIT_STYLE_PROPS = new Set(['font-weight', 'font-style', 'text-decoration', 'text-transform', 'vertical-align']);
...
function styleFormatOverride(style) {
  ...
  if (textTransform === 'lowercase') {
    set |= lexical.TEXT_TYPE_TO_FORMAT.lowercase;
    clear |= lexical.TEXT_TYPE_TO_FORMAT.uppercase | lexical.TEXT_TYPE_TO_FORMAT.capitalize;
  } else if (textTransform === 'uppercase') {
    set |= lexical.TEXT_TYPE_TO_FORMAT.uppercase;
    clear |= lexical.TEXT_TYPE_TO_FORMAT.lowercase | lexical.TEXT_TYPE_TO_FORMAT.capitalize;
  } else if (textTransform === 'capitalize') {
    set |= lexical.TEXT_TYPE_TO_FORMAT.capitalize;
    clear |= lexical.TEXT_TYPE_TO_FORMAT.lowercase | lexical.TEXT_TYPE_TO_FORMAT.uppercase;
  } else if (textTransform === 'none') {
    clear |= lexical.TEXT_TYPE_TO_FORMAT.lowercase | lexical.TEXT_TYPE_TO_FORMAT.uppercase | lexical.TEXT_TYPE_TO_FORMAT.capitalize;
  }
 ...
 * @experimental
 */
const CoreImportExtension = {
  dependencies: [([DOMImportExtension, {
 ...
 */
const HorizontalRuleImportExtension = {
...
function $generateDOMFromNodes(container, selection = null, editor = lexical.$getEditor()) {
  return $withRenderContext([contextValue(RenderContextExport, true)], editor)(() => {
    ...
    // stay in agreement.
    const slotFrame = lexical.$getSelectionSlotFrame(selection);
...
function $generateHtmlFromNodes(editor, selection = null) {
  ...
  return $generateDOMFromNodes(lexical.$getDocument().createElement('div'), selection, editor).innerHTML;
}
...
function $appendTerminatingLineBreak(element, lastIncludedChild) {
  ...
  if (lexical.$isLineBreakNode(lastIncludedChild) && lexical.isHTMLElement(element) && lexical.isBlockDomNode(element) && lastChild !== null && lastChild.nodeName === 'BR') {
    const br = lexical.$getDocument().createElement('br');
...
function $appendNodesToHTML(editor, currentNode, parentElementAppend, selection$1 = null, domConfig = lexical.$getEditorDOMRenderConfig(editor)) {
  ...
  const fragment = lexical.$getDocument().createDocumentFragment();
