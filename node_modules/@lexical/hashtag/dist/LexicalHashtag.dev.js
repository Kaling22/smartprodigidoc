...
class HashtagNode extends lexical.TextNode {
  $config() {
    return this.config('hashtag', {
      extends: lexical.TextNode
    });
  }
  createDOM(config) {
    const element = super.createDOM(config);
    lexical.addClassNamesToElement(element, config.theme.hashtag);
    return element;
  }
  canInsertTextBefore() {
    return false;
...
function registerLexicalHashtag(editor, config = defaultHashtagConfig) {
  return lexical.mergeRegister(...text.registerLexicalTextEntity(editor, config.getHashtagMatch, HashtagNode, textNode => $createHashtagNode(textNode.getTextContent())));
}
/**
