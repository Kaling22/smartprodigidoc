...
const FOCUSABLE_SELECTOR = ['a[href]', 'button:not([disabled])', 'input:not([disabled]):not([type="hidden"])', 'select:not([disabled])', 'textarea:not([disabled])', '[tabindex]:not([tabindex="-1"])', '[contenteditable="true"]'].join(',');
...
function containsComposed(container, target) {
  ...
  while (current !== null) {
    ...
    if (lexical.isDOMShadowRoot(current)) {
...
function registerFocusTrap(container, options = {}) {
  ...
  const deepActive = lexical.getActiveElementDeep(doc);
  const previouslyFocused = lexical.isHTMLElement(deepActive) ? deepActive : null;
  ...
  const keydownHandler = event => {
    ...
    const active = lexical.getActiveElementDeep(doc);
    const activeIndex = lexical.isHTMLElement(active) && containsComposed(container, active) ? currentFocusable.indexOf(active) : -1;
    if (event.shiftKey) {
      (activeIndex <= 0 ? last : currentFocusable[activeIndex - 1]).focus();
    } else {
      (activeIndex === -1 || activeIndex === currentFocusable.length - 1 ? first : currentFocusable[activeIndex + 1]).focus();
    }
  };
  ...
  const focusinHandler = event => {
    ...
    if (!lexical.isHTMLElement(target) || containsComposed(container, target)) {
      return;
    }
  ...
  () => {
    if (previouslyFocused !== null && typeof previouslyFocused.focus === 'function' && containsComposed(doc, previouslyFocused)) {
      previouslyFocused.focus();
    }
  }, lexical.registerEventListener(container, 'keydown', keydownHandler), lexical.registerEventListener(doc, 'focusin', focusinHandler));
}
...
function registerRovingTabIndex(container, options = {}) {
  ...
  const init = () => {
    ...
    const active = lexical.getActiveElementDeep(container.ownerDocument);
  ...
  return lexical.mergeRegister(lexical.registerEventListener(container, 'keydown', handler), () => {
    const items = getItems();
    items.forEach(item => {
      item.tabIndex = 0;
    });
...
const DEFAULT_TOOLBAR_FOCUSABLE_SELECTOR = ':scope > button:not([disabled]), :scope > [tabindex="0"]';
/**
...
function registerFocusManager(editor, toolbar, options = {}) {
  ...
  return lexical.mergeRegister(editor.registerCommand(lexical.KEY_DOWN_COMMAND, event => {
    ...
    firstItem.focus();
    return true;
  }, lexical.COMMAND_PRIORITY_LOW), lexical.registerEventListener(toolbar, 'keydown', handler));
}
...
const AriaLiveRegionExtension = {
  ...
  register(_editor, _config, state) {
    ...
    const rootElement = state.getDependency(extension.RootElementExtension).output;
    const region = extension.signal(null);
    return lexical.mergeRegister(
 ...
 */
const HistoryAnnounceExtension = {
  ...
  register(editor, _config, state) {
    ...
    const {
      announce
    } = state.getDependency(AriaLiveRegionExtension).output;
    ...
    return extension.effect(() => disabled.value ? undefined : lexical.mergeRegister(editor.registerCommand(lexical.UNDO_COMMAND, () => {
      announce(undone.peek());
      return false;
    }, lexical.COMMAND_PRIORITY_LOW), editor.registerCommand(lexical.REDO_COMMAND, () => {
 ...
 * `AriaLiveRegionExtension`'s shared sink.
 */
const EditorModeAnnounceExtension = {
  build: (_editor, config) => extension.namedSignals(config),
  config: {
    disabled: false,
  ...
  register(editor, _config, state) {
    ...
    return extension.effect(() => disabled.value ? undefined : editor.registerEditableListener(isEditable => {
 ...
 */
const FocusTrapExtension = {
  build: () => lexical.createRefCountedRegistry(registerFocusTrap),
 ...
 */
const RovingTabIndexExtension = {
  build: () => lexical.createRefCountedRegistry(registerRovingTabIndex),
 ...
 */
const FocusManagerExtension = {
  build: editor => lexical.createRefCountedRegistry((toolbar, options) => registerFocusManager(editor, toolbar, options)),
  name: '@lexical/a11y/FocusManager',
  register: (_editor, _config, state) => () => state.getOutput().dispose()
};

exports.AriaLiveRegionExtension = AriaLiveRegionExtension;
exports.EditorModeAnnounceExtension = EditorModeAnnounceExtension;
exports.FocusManagerExtension = FocusManagerExtension;
exports.FocusTrapExtension = FocusTrapExtension;
exports.HistoryAnnounceExtension = HistoryAnnounceExtension;
exports.RovingTabIndexExtension = RovingTabIndexExtension;
