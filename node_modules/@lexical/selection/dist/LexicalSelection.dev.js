...
function $getComputedStyleForParent(node) {
  ...
  const parent = lexical.$isRootNode(node) ? node : node.getParent();
...
function $setBlocksType(selection, $createElement, $afterCreateElement = $copyBlockFormatIndent) {
  ...
  for (const prevNode of blockMap.values()) {
    if (lexical.$getSlotHost(prevNode) !== null) {
      continue;
    }
...
function $wrapNodes(selection, createElement, wrappingElement = null) {
  ...
  // selection.)
  if (anchor !== null) {
    const slotFrame = lexical.$getSlotFrame(anchor.getNode());
    if (slotFrame !== null && !lexical.$isRootOrShadowRoot(slotFrame)) {
      return;
    }
  }
