...
class MarkNode extends lexical.ElementNode {
  ...
  createDOM(config) {
    const element = lexical.$getDocument().createElement('mark');
    lexical.addClassNamesToElement(element, config.theme.mark);
    if (this.__ids.length > 1) {
      lexical.addClassNamesToElement(element, config.theme.markOverlap);
    }
    return element;
  }
  updateDOM(prevNode, element, config) {
    ...
    if (hadOverlap !== hasOverlap) {
      if (hasOverlap) {
        lexical.addClassNamesToElement(element, overlapTheme);
      } else {
        lexical.removeClassNamesFromElement(element, overlapTheme);
      }
    }
    return false;
  }
