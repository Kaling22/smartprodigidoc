...
function mlcPositionNodeOnRange(editor, range, onReposition) {
  ...
  let observer = null;
  let lastNodes = [];
  const wrapperNode = lexical.getRootOwnerDocument(editor.getRootElement()).createElement('div');
  ...
  function position() {
    ...
    for (let i = 0; i < rects.length; i++) {
      ...
      const rectNode = lastNodes[i] || lexical.getRootOwnerDocument(rootDOMNode).createElement('div');
...
function selectionAlwaysOnDisplay(editor, onReposition) {
  ...
  return editor.registerRootListener(rootElement => {
    if (rootElement) {
      ...
      const cleanup = lexical.mergeRegister(lexical.registerEventListener(document, 'selectionchange', onSelectionChange), () => {
        if (removeSelectionMark !== null) {
          removeSelectionMark();
        }
      });
...
function $hasContentAfter(caret) {
  return lexical.$isExtendableTextPointCaret(caret) || lexical.$getAdjacentSiblingOrParentSiblingCaret(lexical.$isTextPointCaret(caret) ? caret.getSiblingCaret() : caret, 'shadowRoot') !== null;
}
...
function $insertNodeToNearestRoot(node) {
  ...
  const insertCaret = lexical.$insertNodeToNearestRootAtCaret(node, initialCaret, $hasContentAfter(initialCaret) ? {
...
exports.$isBlockFullySelected = lexical.$isBlockFullySelected;
