...
import { getRootOwnerDocument, getDOMShadowRoots, isDOMShadowRoot, getParentElement, $getEditor, shallowMergeConfig, $isRangeSelection, $getSelection, $setSelection, tokenizeRawText, $createTabNode, $getRoot, $getSelectionSlotFrame, $isElementNode, $parseSerializedNode, SELECTION_INSERT_CLIPBOARD_NODES_COMMAND, getDOMSelection, COPY_COMMAND, COMMAND_PRIORITY_CRITICAL, $isTextNode, $isNodeSelection, $getSlotNames, $getSlot, $splitAtPointCaretNext, $setSelectionFromCaretRange, $getCollapsedCaretRange, $caretFromPoint, $isTextPointCaret, $getCaretRange, $getChildCaret, getDOMSelectionPoints, isSelectionWithinEditor, $getNearestNodeFromDOMNode, $getTextPointCaret, $getChildCaretAtIndex, $getCaretRangeInDirection, $caretRangeFromSelection, $comparePointCaretNext, findAllLexicalElementsDeep, getEditorPropertyFromDOMNode, isLexicalEditor, isHTMLElement } from 'lexical';
import { $getPeerDependency, getPeerDependencyFromEditor } from '@lexical/extension';
import { CoreImportExtension, $generateNodesFromDOMViaExtension, contextValue, ImportSource, ImportSourceDataTransfer, $generateNodesFromDOM, $generateHtmlFromNodes } from '@lexical/html';
 ...
 * ```
 */
const ClipboardImportExtension = {
 ...
 * ```
 */
const ClipboardDOMImportExtension = {
  dependencies: [CoreImportExtension, ([ClipboardImportExtension, {
    $importMimeType: {
...
function findEditorRootByKey(key, doc) {
  for (const el of findAllLexicalElementsDeep(doc)) {
    ...
    if (isLexicalEditor(editor) && editor.getKey() === key && isHTMLElement(el)) {
      return el;
    }
  }
  return null;
 ...
 * ```
 */
const GetClipboardDataExtension = {
