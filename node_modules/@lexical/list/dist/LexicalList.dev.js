 ...
 * for the same reason.
 */
function $newListFrom(list, listType) {
  return $isListNode(list) ? lexical.$copyNode(list).setListType(listType) : $createListNode(listType);
}
...
function $insertList(listType) {
  ...
  if (selection !== null) {
    ...
    if (lexical.$isRangeSelection(selection)) {
      ...
      if (lexical.$isRootOrShadowRoot(anchorNode)) {
        ...
        if (lexical.$isRootOrShadowRoot(anchorNodeParent)) {
          ...
          if (lexical.$getSlotHost(parent) === null) {
    ...
    for (let i = 0; i < nodes.length; i++) {
      const node = nodes[i];
      if (lexical.$isElementNode(node) && node.isEmpty() && !$isListItemNode(node) &&
      ...
      lexical.$getSlotHost(node) === null && !handled.has(node.getKey())) {
      ...
      while (parent != null) {
        ...
        if ($isListNode(parent)) {
          ...
          // slot.
          if (!handled.has(parentKey) && lexical.$getSlotHost(parent) === null) {
...
function mergeLists(list1, list2) {
  ...
  listItem1.getFirstChild().getListType() === listItem2.getFirstChild().getListType()) {
...
function $removeList() {
  ...
  if (lexical.$isRangeSelection(selection)) {
    ...
    for (const listNode of listNodes) {
      ...
      // owns it.
      if (lexical.$getSlotHost(listNode) !== null) {
        continue;
      }
      ...
      for (const listItemNode of listItems) {
        ...
        paragraph.setFormat(listItemNode.getFormatType()).setIndent(listItemNode.getIndent()).setDirection(listItemNode.getDirection()).setStyle(listItemNode.getStyle());
...
class ListItemNode extends lexical.ElementNode {
  ...
  insertAfter(node, restoreSelection = true) {
    ...
    if (siblings.length !== 0) {
      ...
      if (newListNode.getListType() === 'number' && $isListItemNode(firstSibling)) {
  ...
  remove(preserveEmptyParent) {
    ...
    prevSibling.getFirstChild().getListType() === nextSibling.getFirstChild().getListType()) {
  ...
  splice(start, deleteCount, nodesToInsert) {
    ...
    for (let i = 0; i < nodesToInsert.length; i++) {
      ...
      if (!$isListItemNode(node)) {
        ...
        if (lexical.$isElementNode(node) && !($isListNode(node) || node.isInline())) {
    ...
    if (listLevelClassName !== undefined) {
      ...
      for (let i = 0; i < listLevelsClassNames.length; i++) {
        if (i !== normalizedListDepth) {
          classesToRemove.push(...lexical.normalizeClassNames(listLevelsClassNames[i]));
        }
      }
    }
...
const CHECKBOX_KEYS = new Set([' ', 'Alt', 'ArrowDown', 'ArrowLeft', 'ArrowUp', 'CapsLock', 'Control', 'Escape', 'Meta', 'Shift']);
...
function registerCheckList(editor, options) {
  ...
  const configHandleSelectDefaults = event => {
    lastPress = handleSelectDefaults(event, peekDisableTakeFocusOnClick()) ? 'checkMark' : 'content';
  };
  ...
  return lexical.mergeRegister(editor.registerCommand(INSERT_CHECK_LIST_COMMAND, () => {
    ...
    if (rootElement !== null) {
      ...
      lexical.registerEventListeners(rootElement, {
        ...
        focusin: () => {
          if (lexical.IS_FIREFOX && lastPress === 'content') {
            returnFocusToRoot(editor);
          }
        },
...
function returnFocusToRoot(editor) {
  const rootElement = editor.getRootElement();
  const activeElement = rootElement ? lexical.getActiveElement(rootElement) : null;
  if (rootElement !== null && lexical.isHTMLElement(activeElement) && isCheckListItem(activeElement)) {
    rootElement.focus({
      preventScroll: true
    });
  }
}
function isCheckListItem(dom) {
  const parentNode = dom.parentNode;
  return dom.tagName === 'LI' && parentNode != null &&
  // @ts-ignore internal field
...
function getActiveCheckListItem(editor) {
  ...
  const rootElement = editor.getRootElement();
  const activeElement = rootElement ? lexical.getActiveElement(rootElement) : null;
  return lexical.isHTMLElement(activeElement) && isCheckListItem(activeElement) ? activeElement : null;
}
...
function handleArrowUpOrDown(event, editor, backward) {
  ...
  if (activeItem != null) {
    ...
    editor.update(() => {
      ...
      onUpdate: () => {
        const dom = nextItemKey === null ? null : editor.getElementByKey(nextItemKey);
        if (dom != null) {
          dom.focus();
        }
      }
 ...
 */
function $normalizeListChildren(children, listNode) {
  const out = [];
...
const ListRule = {
  $import: (ctx, el) => {
    ...
    return [node.splice(0, 0, html.$propagateTextAlignToBlockChildren($normalizeListChildren(ctx.$importChildren(el), node), el))];
  },
  match: /* @__PURE__ */html.sel.tag('ol', 'ul'),
...
const ListItemRule = {
  ...
  match: /* @__PURE__ */html.sel.tag('li'),
...
const TaskListItemRule = {
  ...
  match: /* @__PURE__ */html.sel.tag('li').classAll('task-list-item'),
  name: '@lexical/list/li-task-list-item'
};
const JoplinChecklistItemRule = {
  $import: (ctx, el, $next) => {
  ...
  match: /* @__PURE__ */html.sel.tag('li').classAll('joplin-checkbox'),
...
function stripWordMarker(el) {
  ...
  let outer = inner;
  while (outer.parentElement && outer.parentElement !== el && outer.parentElement.nodeName === 'SPAN') {
    outer = outer.parentElement;
  }
  outer.remove();
}
function isWordListParagraph(node) {
  return lexical.isHTMLElement(node) && WORD_LIST_CLASS_RE.test(node.className);
}
function $buildWordListTree(ctx, items) {
  ...
  for (const item of items) {
    ...
    stack[stack.length - 1].list.append($createListItemNode().splice(0, 0, ctx.$importChildren(item.el, {
      schema: html.InlineSchema
    })));
  }
  return root;
}
 ...
 */
const WordListConsumed = /* @__PURE__ */html.createImportState('@lexical/list/word-consumed-list-items', () => null);
const WordListParagraphRule = {
  $import: (ctx, el) => {
    ...
    while (cur && isWordListParagraph(cur)) {
      ...
      if (cur.classList.contains('MsoListParagraphCxSpLast') || cur.className === 'MsoListParagraph') {
        break;
      }
  ...
  match: /* @__PURE__ */html.sel.tag('p').classAny('MsoListParagraph', 'MsoListParagraphCxSpFirst', 'MsoListParagraphCxSpMiddle', 'MsoListParagraphCxSpLast'),
...
const WordOfficeParagraphRule = {
  $import: () => [],
  match: /* @__PURE__ */html.sel.tag('o:p'),
  name: '@lexical/list/word-o-p'
};
const WordListPasteOverlay = /* @__PURE__ */html.defineOverlayRules([WordOfficeParagraphRule, WordListParagraphRule]);
 ...
 */
const $installWordListPasteOverlay = (dom, ctx, $next) => {
  ...
  if (meta && WORD_GENERATOR_RE.test(meta.getAttribute('content') || '')) {
    for (const el of Array.from(dom.querySelectorAll('[style*="mso-list"]'))) {
      const msoList = lexical.getStyleObjectFromCSS(el.getAttribute('style') || '')['mso-list'];
      if (msoList) {
        el.setAttribute(MSO_LIST_DATA_ATTR, msoList);
      }
    }
    ctx.session.update(html.ImportOverlays, prev => [...prev, WordListPasteOverlay]);
  }
  $next();
};
 ...
 * @experimental
 */
const WordListImportExtension = {
  dependencies: [
  // The overlay builds ListNodes, so the nodes have to be registered.
  ListExtension, ([html.DOMImportExtension, {
...
exports.WordListImportExtension = WordListImportExtension;
