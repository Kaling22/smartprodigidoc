...
function $newListFrom(list, listType) {
  return $isListNode(list) ? $copyNode(list).setListType(listType) : $createListNode(listType);
}
...
function $insertList(listType) {
  ...
  if (selection !== null) {
    ...
    for (let i = 0; i < nodes.length; i++) {
      const node = nodes[i];
      if ($isElementNode(node) && node.isEmpty() && !$isListItemNode(node) &&
      ...
      $getSlotHost(node) === null && !handled.has(node.getKey())) {
...
function registerCheckList(editor, options) {
  ...
  return mergeRegister(editor.registerCommand(INSERT_CHECK_LIST_COMMAND, () => {
    ...
    if (rootElement !== null) {
      return mergeRegister(registerEventListeners(rootElement, {
      ...
      registerEventListener(rootElement, 'touchstart', configHandleSelectDefaults, {
        capture: true,
        passive: false
      }));
    }
...
function returnFocusToRoot(editor) {
  ...
  if (rootElement !== null && isHTMLElement(activeElement) && isCheckListItem(activeElement)) {
    rootElement.focus({
      preventScroll: true
    });
  }
...
function getActiveCheckListItem(editor) {
  ...
  const rootElement = editor.getRootElement();
  const activeElement = rootElement ? getActiveElement(rootElement) : null;
  return isHTMLElement(activeElement) && isCheckListItem(activeElement) ? activeElement : null;
}
...
const ListRule = {
  $import: (ctx, el) => {
    ...
    return [node.splice(0, 0, $propagateTextAlignToBlockChildren($normalizeListChildren(ctx.$importChildren(el), node), el))];
  },
  match: /* @__PURE__ */sel.tag('ol', 'ul'),
...
const TaskListItemRule = {
  ...
  match: /* @__PURE__ */sel.tag('li').classAll('task-list-item'),
...
const JoplinChecklistItemRule = {
  ...
  match: /* @__PURE__ */sel.tag('li').classAll('joplin-checkbox'),
 ...
 */
const WordListConsumed = /* @__PURE__ */createImportState('@lexical/list/word-consumed-list-items', () => null);
const WordListParagraphRule = {
  ...
  match: /* @__PURE__ */sel.tag('p').classAny('MsoListParagraph', 'MsoListParagraphCxSpFirst', 'MsoListParagraphCxSpMiddle', 'MsoListParagraphCxSpLast'),
...
const WordListPasteOverlay = /* @__PURE__ */defineOverlayRules([WordOfficeParagraphRule, WordListParagraphRule]);
...
const $installWordListPasteOverlay = (dom, ctx, $next) => {
  ...
  if (meta && WORD_GENERATOR_RE.test(meta.getAttribute('content') || '')) {
    for (const el of Array.from(dom.querySelectorAll('[style*="mso-list"]'))) {
      const msoList = getStyleObjectFromCSS(el.getAttribute('style') || '')['mso-list'];
      if (msoList) {
