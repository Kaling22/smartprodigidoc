...
class TableCellNode extends lexical.ElementNode {
  ...
  createDOM(config) {
    const element = lexical.$getDocument().createElement(this.getTag());
    if (this.__width) {
    ...
    lexical.addClassNamesToElement(element, config.theme.tableCell, this.hasHeader() && config.theme.tableCellHeader);
    return element;
  }
  exportDOM(editor) {
...
function $convertTableCellNodeElement(domNode) {
  ...
  lexical.$setDirectionFromDOM(tableCellNode, domNode_);
...
const INSERT_TABLE_COMMAND = /* @__PURE__ */lexical.createCommand('INSERT_TABLE_COMMAND');
...
class TableRowNode extends lexical.ElementNode {
  ...
  createDOM(config) {
    const element = lexical.$getDocument().createElement('tr');
    if (this.__height) {
      element.style.height = `${this.__height}px`;
    }
    lexical.addClassNamesToElement(element, config.theme.tableRow);
    return element;
  }
...
function $convertTableRowElement(domNode) {
  ...
  return {
    ...
    node: lexical.$setDirectionFromDOM($createTableRowNode(height), domNode_)
  };
}
...
function $getTableCellNodeFromLexicalNode(startingNode) {
  const node = lexical.$findMatchingParent(startingNode, n => $isTableCellNode(n));
...
function $getTableRowNodeFromTableCellNodeOrThrow(startingNode) {
  const node = lexical.$findMatchingParent(startingNode, n => $isTableRowNode(n));
...
function $getTableNodeFromLexicalNodeOrThrow(startingNode) {
  const node = lexical.$findMatchingParent(startingNode, n => $isTableNode(n));
  if ($isTableNode(node)) {
    return node;
  }
...
function $deleteTableRowAtSelection() {
  ...
  for (let row = focusEndRow; row >= anchorStartRow; row--) {
    for (let column = columnCount - 1; column >= 0; column--) {
      ...
      row === focusEndRow) {
        if (!$isTableRowNode(nextRowNode)) {
          formatDevErrorMessage(`Expected a TableRowNode`);
        }
        let insertAfterCell = null;
...
function $unmergeCell() {
  ...
  const anchor = selection.anchor.getNode();
  const cellNode = lexical.$findMatchingParent(anchor, $isTableCellNode);
...
function $unmergeCellNode(cellNode) {
  ...
  const $createSplitCell = headerState => lexical.$copyNode(cell).setColSpan(1).setRowSpan(1).setHeaderStyles(headerState).setWidth(undefined).append(lexical.$createParagraphNode());
  if (colSpan > 1) {
    ...
    for (let i = 1; i < rowSpan; i++) {
      ...
      if (insertAfterCell === null) {
        for (let j = colSpan - 1; j >= 0; j--) {
          $insertFirst(currentRowNode, $createSplitCell(colStyles[j] | rowStyles[i]));
        }
      } else {
        for (let j = colSpan - 1; j >= 0; j--) {
          insertAfterCell.insertAfter($createSplitCell(colStyles[j] | rowStyles[i]));
        }
      }
    }
    cell.setRowSpan(1);
...
function $getNodeTriplet(source) {
  ...
  if (source instanceof TableCellNode) {
    ...
    const cell_ = lexical.$findMatchingParent(source, $isTableCellNode);
    if (!$isTableCellNode(cell_)) {
      formatDevErrorMessage(`Expected to find a parent TableCellNode`);
    }
    cell = cell_;
  } else {
    const cell_ = lexical.$findMatchingParent(source.getNode(), $isTableCellNode);
    if (!$isTableCellNode(cell_)) {
      formatDevErrorMessage(`Expected to find a parent TableCellNode`);
    }
    cell = cell_;
 ...
 */
function $moveTableRow(tableNode, originRow, targetRow) {
  if (originRow === targetRow) {
    return;
  }
  const rows = tableNode.getChildren().filter($isTableRowNode);
  const rowCount = rows.length;
  if (originRow < 0 || originRow >= rowCount || targetRow < 0 || targetRow >= rowCount) {
    return;
  }
  if (!$isSimpleTable(tableNode)) {
    return;
...
function $insertTableIntoGrid(tableNode, selection) {
  ...
  const focusCellNode = lexical.$findMatchingParent(focus.getNode(), n => $isTableCellNode(n));
  ...
  if (isTableSel && didPerformMergeOperations) {
...
function $setTableRowIsHeader(tableNode, rowIndex, isHeader) {
  const [gridMap] = $computeTableMapSkipCellCheck(tableNode, null, null);
  if (!(rowIndex >= 0 && rowIndex < gridMap.length)) {
    formatDevErrorMessage(`Table row index ${String(rowIndex)} out of range`);
  }
...
function $setTableColumnIsHeader(tableNode, columnIndex, isHeader) {
  const [gridMap] = $computeTableMapSkipCellCheck(tableNode, null, null);
  if (!(gridMap.length > 0 && columnIndex >= 0 && columnIndex < gridMap[0].length)) {
    formatDevErrorMessage(`Table column index ${String(columnIndex)} out of range`);
  }
  const headerState = isHeader ? TableCellHeaderStates.COLUMN : TableCellHeaderStates.NO_STATUS;
  const visited = new Set();
...
function $getCellNodes(tableSelection) {
  const [[anchorNode, anchorCell, anchorRow, anchorTable], [focusNode, focusCell, focusRow, focusTable]] = ['anchor', 'focus'].map(k => {
    ...
    const cellNode = lexical.$findMatchingParent(node, $isTableCellNode);
    if (!$isTableCellNode(cellNode)) {
  ...
  insertRawText(text) {
    ...
    for (const row of tsvGrid) {
      ...
      for (const cellText of row) {
        ...
        if (cellText) {
          paragraph.append(lexical.$createTextNode(cellText));
        }
...
class TableObservers {
  ...
  $getTableNodesAndObservers() {
    const tableNodesAndObservers = [];
    for (const [tableKey, [tableObserver]] of Array.from(this.observers.entries())) {
...
class TableObserver {
  ...
  $enableHighlightStyle() {
    ...
    lexical.removeClassNamesFromElement(tableElement, editor._config.theme.tableSelection);
  ...
  $disableHighlightStyle() {
    ...
    lexical.addClassNamesToElement(tableElement, this.editor._config.theme.tableSelection);
  ...
  $getAnchorTableCell() {
    const node = this.anchorCellNodeKey ? lexical.$getNodeByKey(this.anchorCellNodeKey) : null;
  ...
  $getFocusTableCell() {
    const node = this.focusCellNodeKey ? lexical.$getNodeByKey(this.focusCellNodeKey) : null;
    return $isTableCellNode(node) ? node : null;
  }
  ...
  $clearText() {
    ...
    if (isEntireTableSelected) {
      ...
      if (lexical.$isRootNode(parent) && parent.isEmpty()) {
        editor.dispatchCommand(lexical.INSERT_PARAGRAPH_COMMAND);
      }
      return;
    }
    selectedNodes.forEach(cellNode => {
      if (lexical.$isElementNode(cellNode)) {
        ...
        const paragraphNode = lexical.$isParagraphNode(firstChild) ? lexical.$copyNode(firstChild) : lexical.$createParagraphNode();
...
function $getTableNodeByKeyOrThrow(key) {
  const tableNode = lexical.$getNodeByKeyOrThrow(key);
  if (!$isTableNode(tableNode)) {
    formatDevErrorMessage(`Expected TableNode for key ${key}`);
  }
  return tableNode;
}
...
function registerTableWindowHandlers(editor, tableObservers) {
  ...
  return editor.registerRootListener(rootElement => {
    ...
    return lexical.registerEventListener(editorWindow, 'pointerdown', pointerDownCallback);
  });
}
function $handleTableClick(editor, event, selectedDOMCell, tableElement, tableObserver, tableObservers) {
  ...
  const createPointerHandlers = startingCell => {
    ...
    const isGesturePointer = pointerEvent => pointerEvent.pointerId === gesturePointerId;
    ...
    const isBeyondTapSlop = (clientX, clientY) => !(Math.abs(clientX - startClientX) <= TAP_SLOP && Math.abs(clientY - startClientY) <= TAP_SLOP);
    const stopSelecting = () => {
    ...
    const applyFocusCell = (focusCell, override) => {
      ...
      if (!hasAnchorForGesture && (!tableObserver.isPointerDrag || focusCell.elem === startingCell.elem)) {
        return;
      }
    ...
    const maybeStartAutoScroll = () => {
      ...
      // pointer type.
      if (autoScrollRafId !== null || isTouch && !tableObserver.isPointerDrag || !isNearScrollEdge()) {
        return;
      }
      autoScrollRafId = editorWindow.requestAnimationFrame(tickAutoScroll);
    };
    ...
    const onPointerGestureEnd = endEvent => {
    ...
    const onPointerMove = moveEvent => {
      ...
      if (!tableObserver.isPointerDrag && isBeyondTapSlop(lastClientX, lastClientY)) {
      ...
      const override = !(lexical.IS_FIREFOX || tableElement.contains(moveTarget));
      if (override) {
    ...
    removeGestureListeners = lexical.registerEventListeners(editorWindow, {
  ...
  const prevAnchorNestedCell = lexical.$isRangeSelection(prevSelection) || $isTableSelection(prevSelection) ? $findCellNode(prevSelection.anchor.getNode()) : null;
  const isShiftClickIntoNestedTable = prevAnchorNestedCell !== null && prevAnchorNestedCell.isParentOf(tableNode);
  if (lexical.IS_FIREFOX && event.shiftKey && (lexical.$isRangeSelection(prevSelection) || $isTableSelection(prevSelection)) && ($isSelectionInTable(prevSelection, tableNode) || isShiftClickIntoNestedTable)) {
    const prevAnchorNode = prevSelection.anchor.getNode();
    const prevAnchorCell = $findParentTableCellNodeInTable(tableNode, prevSelection.anchor.getNode());
    if (prevAnchorCell) {
      const prevAnchorDOMCell = $getObserverCellFromCellNodeOrThrow(tableObserver, prevAnchorCell);
...
function applyTableHandlers(tableNode, element, editor, hasTabHandler, tableObservers) {
  ...
  const onTripleClick = event => {
    const target = lexical.getComposedEventTarget(event);
    if (event.detail >= 3 && lexical.isDOMNode(target)) {
  ...
  tableObserver.listenersToRemove.add(lexical.registerEventListener(tableElement, 'mousedown', onTripleClick, tableObserver.listenerOptions));
  ...
  tableObserver.listenersToRemove.add(lexical.registerEventListener(doc, 'paste', event => {
    if (event.defaultPrevented) {
      return;
    }
    const shouldIntercept = editor.read('latest', () => {
      const selection = lexical.$getSelection();
      return rootElement.contains(doc.activeElement) && $isTableSelection(selection) && $isSelectionInTable(selection, tableNode);
    });
    if (shouldIntercept) {
      event.preventDefault();
  ...
  tableObserver.listenersToRemove.add(editor.registerCommand(lexical.FORMAT_ELEMENT_COMMAND, formatType => {
    ...
    if (minRow === 0 && minColumn === 0 && maxRow === tableMap.length - 1 && maxColumn === tableMap[0].length - 1) {
...
function $handleTableSelectionChangeCommand(tableObservers, editor) {
  ...
  if (!!shouldCheckSelectionForTable && lexical.$isRangeSelection(prevSelection) && lexical.$isRangeSelection(selection) && selection.isCollapsed()) {
    ...
    const tableNode = lexical.$getNodeByKey(shouldCheckSelectionForTable);
    if ($isTableNode(tableNode)) {
      const anchor = selection.anchor.getNode();
      const firstRow = tableNode.getFirstChild();
      ...
      if (anchorCell !== null && $isTableRowNode(firstRow)) {
        ...
        if ($isTableCellNode(firstCell) && tableNode.is(lexical.$findMatchingParent(anchorCell, node => node.is(tableNode) || node.is(firstCell)))) {
  ...
  for (const [tableNode, tableObserver] of tableObservers.$getTableNodesAndObservers()) {
...
function $fixRangeSelectionForSelectedTable(selection, tableObservers) {
  ...
  if (shouldMoveFocus) {
    ...
    // table selection (#8538).
    if (tableObserver.pointerType === 'touch' && tableObserver.isSelecting && tableObserver.isPointerDrag && selection.isCollapsed() && lexical.$isRangeSelection(prevSelection) && prevSelection.isCollapsed()) {
      ...
      const hasLeftStartCell = pointerStartCell !== null && $getObserverCellFromCellNodeOrThrow(tableObserver, focusCellNode).elem !== pointerStartCell.elem;
      if (hasLeftStartCell && prevAnchorCellNode && !prevAnchorCellNode.is(focusCellNode)) {
...
function $fixTableSelectionForSelectedTable(editor, selection) {
  ...
  const points = domSelection && lexical.getDOMSelectionPoints(domSelection, editor.getRootElement());
  if (domSelection && points && points.anchorNode && points.focusNode) {
    const focusNode = lexical.$getNearestNodeFromDOMNode(points.focusNode);
    const isFocusOutside = focusNode && !tableNode.isParentOf(focusNode);
    const anchorNode = lexical.$getNearestNodeFromDOMNode(points.anchorNode);
...
function $selectAdjacentCell(tableCellNode, direction) {
  ...
  const parentRow = lexical.$findMatchingParent(tableCellNode, $isTableRowNode);
  if (!(parentRow !== null)) {
  ...
  const parentTable = lexical.$findMatchingParent(parentRow, $isTableNode);
  if (!(parentTable !== null)) {
 ...
 * instead.
 */
function getAnchorCorner(rect, anchorCellValue, focusCellValue) {
...
function $extractRectCorners(tableMap, anchorCellValue, newFocusCellValue) {
  ...
  const anchorCorner = getAnchorCorner(rect, anchorCellValue, newFocusCellValue);
...
function $addHighlightToDOM(editor, cell) {
  ...
  lexical.addClassNamesToElement(element, editorThemeClasses.tableCellSelected);
}
function $removeHighlightFromDOM(editor, cell) {
  ...
  const editorThemeClasses = editor._config.theme;
  lexical.removeClassNamesFromElement(element, editorThemeClasses.tableCellSelected);
}
...
function $findTableNode(node) {
  const tableNode = lexical.$findMatchingParent(node, $isTableNode);
...
function $handleHorizontalArrowKeyRangeSelection(editor, event, selection, alter, isBackward, tableNode, tableObserver) {
  ...
  const anchorCellTable = lexical.$findMatchingParent(anchorCell, $isTableNode);
 ...
 */
function $isSelectionBeforeTable(selection, tableNode) {
  if (!lexical.$isRangeSelection(selection)) {
    return false;
  }
  const focusCaret = lexical.$caretFromPoint(selection.focus, 'next');
  ...
  const tableCaret = lexical.$getChildCaret(tableNode, 'next');
  return lexical.$getCommonAncestor(focusCaret.origin, tableCaret.origin) !== null && lexical.$comparePointCaretNext(focusCaret, tableCaret) < 0;
}
function $handleArrowKey(editor, event, direction, tableNode, tableObserver, tableObservers) {
  ...
  if (!$isSelectionInTable(selection, tableNode)) {
    if (lexical.$isRangeSelection(selection)) {
      if (direction === 'backward') {
        ...
        if (isTableUnselect) {
          let focusParentNode = lexical.$findMatchingParent(focusNode, n => $isTableNode(n));
          if ($isTableCellNode(focusParentNode)) {
            focusParentNode = lexical.$findMatchingParent(focusParentNode, $isTableNode);
          }
          ...
          let focusParentNode = lexical.$findMatchingParent(focusNode, n => lexical.$isElementNode(n) && !n.isInline());
          if ($isTableCellNode(focusParentNode)) {
            focusParentNode = lexical.$findMatchingParent(focusParentNode, $isTableNode);
          }
          if (!focusParentNode) {
  ...
  if (lexical.$isRangeSelection(selection)) {
    ...
    if (selection.isCollapsed()) {
      ...
      const anchorCellNode = lexical.$findMatchingParent(anchor.getNode(), $isTableCellNode);
      const focusCellNode = lexical.$findMatchingParent(focus.getNode(), $isTableCellNode);
      ...
      if (anchor.type === 'element') {
        ...
        const range = lexical.getDOMSelectionRange(domSelection, editor.getRootElement());
        if (range === null) {
          return false;
        }
...
function $getTableEdgeCursorPosition(editor, selection, tableNode) {
  ...
  const domAnchorNode = lexical.getDOMSelectionPoints(domSelection, editor.getRootElement()).anchorNode;
  ...
  const anchorCellNode = lexical.$findMatchingParent(selection.anchor.getNode(), n => $isTableCellNode(n));
  if (!anchorCellNode) {
    return undefined;
  }
  const parentTable = lexical.$findMatchingParent(anchorCellNode, n => $isTableNode(n));
...
function isHTMLDivElement(element) {
  return lexical.isHTMLElement(element) && element.nodeName === 'DIV';
}
function $updateColgroup(dom, colCount, colWidths) {
  let colGroup = dom.querySelector(':scope > colgroup');
  if (!colWidths) {
    if (colGroup) colGroup.remove();
    return;
  }
  if (!colGroup) {
    colGroup = lexical.$getDocument().createElement('colgroup');
    lexical.setDOMUnmanaged(colGroup);
    dom.insertBefore(colGroup, dom.firstChild);
  }
  const cols = [];
  for (let i = 0; i < colCount; i++) {
    const col = lexical.$getDocument().createElement('col');
...
function setRowStriping(dom, config, rowStriping) {
  if (rowStriping) {
    lexical.addClassNamesToElement(dom, config.theme.tableRowStriping);
    dom.setAttribute('data-lexical-row-striping', 'true');
  } else {
    lexical.removeClassNamesFromElement(dom, config.theme.tableRowStriping);
    dom.removeAttribute('data-lexical-row-striping');
  }
}
function setFrozenColumns(dom, tableElement, config, frozenColumnCount) {
  if (frozenColumnCount > 0) {
    lexical.addClassNamesToElement(dom, config.theme.tableFrozenColumn);
    tableElement.setAttribute('data-lexical-frozen-column', 'true');
  } else {
    lexical.removeClassNamesFromElement(dom, config.theme.tableFrozenColumn);
    tableElement.removeAttribute('data-lexical-frozen-column');
  }
}
function setFrozenRows(dom, tableElement, config, frozenRowCount) {
  if (frozenRowCount > 0) {
    lexical.addClassNamesToElement(dom, config.theme.tableFrozenRow);
    tableElement.setAttribute('data-lexical-frozen-row', 'true');
  } else {
    lexical.removeClassNamesFromElement(dom, config.theme.tableFrozenRow);
...
function setScrollableTablesActive(editor, active) {
  if (active) {
    if (editor._config.theme.tableScrollableWrapper === undefined) {
...
class TableNode extends lexical.ElementNode {
  ...
  getDOMSlot(element) {
    ...
    return super.getDOMSlot(element).withElement(tableElement).withAfter(tableElement.querySelector(':scope > colgroup'));
  }
  createDOM(config, editor) {
    const tableElement = lexical.$getDocument().createElement('table');
    if (this.__style) {
      lexical.setDOMStyleFromCSS(tableElement.style, this.__style);
    }
    const colWidths = this.getColWidths();
    if (colWidths) {
      const colGroup = lexical.$getDocument().createElement('colgroup');
  ...
  exportDOM(editor) {
    ...
    const exportedElement = !isHTMLTableElement(element) && lexical.isHTMLElement(element) ? element.querySelector('table') : element;
    ...
    if (lexical.isHTMLElement(element) && isHTMLTableElement(exportedElement) && exportedElement !== element) {
      if (element.dir) {
    ...
    return {
      after: tableElement => {
        ...
        if (!isHTMLTableElement(tableElement) && lexical.isHTMLElement(tableElement)) {
        ...
        if (rows.length > 0) {
          const tBody = lexical.$getDocument().createElement('tbody');
          for (const row of rows) {
...
function $convertTableElement(domNode) {
  ...
  lexical.$setDirectionFromDOM(tableNode, domNode);
  return {
...
function $tableTransform(node) {
  ...
  for (let i = 0; i < gridMap.length; ++i) {
    ...
    const headerState = $isTableCellNode(lastCell) ? lastCell.getHeaderStyles() & TableCellHeaderStates.ROW : TableCellHeaderStates.NO_STATUS;
...
function registerTableSelectionObserver(editor, hasTabHandler = true) {
  ...
  return lexical.mergeRegister(registerTableWindowHandlers(editor, tableObservers), editor.registerCommand(lexical.SELECTION_CHANGE_COMMAND, () => {
...
function registerTablePlugin(editor, options) {
  ...
  return lexical.mergeRegister(editor.registerCommand(INSERT_TABLE_COMMAND, payload => {
...
function $tableSelectionInsertClipboardNodesCommand(selectionPayload, hasNestedTables) {
  ...
  if (!hasTables) {
    if ($isTableSelection(selection)) {
      ...
      for (const node of nodes) {
        const isBlock = lexical.$isElementNode(node) && !node.isInline();
  ...
  const isSelectionInsideOfGrid = isRangeSelection && lexical.$findMatchingParent(selection.anchor.getNode(), n => $isTableCellNode(n)) !== null && lexical.$findMatchingParent(selection.focus.getNode(), n => $isTableCellNode(n)) !== null || isTableSelection;
...
const TableRule = {
  ...
  match: /* @__PURE__ */html.sel.tag('table'),
...
const TableRowRule = {
  ...
  match: /* @__PURE__ */html.sel.tag('tr'),
...
const TableCellRule = {
  ...
  match: /* @__PURE__ */html.sel.tag('td', 'th'),
...
exports.$insertTableColumnAtNode = $insertTableColumnAtNode;
...
exports.$insertTableRowAtNode = $insertTableRowAtNode;
...
exports.$setTableColumnIsHeader = $setTableColumnIsHeader;
exports.$setTableRowIsHeader = $setTableRowIsHeader;
exports.$unmergeCell = $unmergeCell;
exports.$unmergeCellNode = $unmergeCellNode;
