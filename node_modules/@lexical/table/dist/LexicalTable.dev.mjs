...
import { ElementNode, $getDocument, addClassNamesToElement, isHTMLElement, $setDirectionFromDOM, $isInlineElementOrDecoratorNode, $isTextNode, $isLineBreakNode, $createParagraphNode, $applyNodeReplacement, createCommand, $createTextNode, $getSelection, $isRangeSelection, $findMatchingParent, $isParagraphNode, $copyNode, $createPoint, $getNodeByKey, $isElementNode, $normalizeSelection__EXPERIMENTAL, isCurrentlyReadOnlyMode, TEXT_TYPE_TO_FORMAT, $getEditor, $setSelection, SELECTION_CHANGE_COMMAND, removeClassNamesFromElement, getDOMSelection, $createRangeSelection, $isRootNode, INSERT_PARAGRAPH_COMMAND, registerEventListener, KEY_ARROW_DOWN_COMMAND, KEY_ARROW_UP_COMMAND, KEY_ARROW_LEFT_COMMAND, KEY_ARROW_RIGHT_COMMAND, COMMAND_PRIORITY_HIGH, KEY_ESCAPE_COMMAND, DELETE_WORD_COMMAND, DELETE_LINE_COMMAND, DELETE_CHARACTER_COMMAND, KEY_BACKSPACE_COMMAND, KEY_DELETE_COMMAND, CUT_COMMAND, PASTE_COMMAND, getComposedEventTarget, isDOMNode, getActiveElement, COPY_COMMAND, FORMAT_TEXT_COMMAND, FORMAT_ELEMENT_COMMAND, CONTROLLED_TEXT_INSERTION_COMMAND, KEY_TAB_COMMAND, FOCUS_COMMAND, $getNearestNodeFromDOMNode, $isRootOrShadowRoot, getDOMSelectionRange, getDOMSelectionPoints, $getPreviousSelection, $caretFromPoint, $getChildCaret, $getCommonAncestor, $comparePointCaretNext, $isExtendableTextPointCaret, $extendCaretToRange, $isSiblingCaret, $getSiblingCaret, $setPointFromCaret, $normalizeCaret, $createRangeSelectionFromDom, $isChildCaret, $getAdjacentChildCaret, IS_FIREFOX, $getNodeByKeyOrThrow, registerEventListeners, isDOMDocumentNode, isDOMShadowRoot, setDOMStyleFromCSS, setDOMUnmanaged, mergeRegister, COMMAND_PRIORITY_EDITOR, SELECTION_INSERT_CLIPBOARD_NODES_COMMAND, SELECT_ALL_COMMAND, COMMAND_PRIORITY_LOW, CLICK_COMMAND, $getRoot, isHTMLTableRowElement, IS_BOLD, IS_ITALIC, IS_UNDERLINE, IS_STRIKETHROUGH, $fullReconcile } from 'lexical';
import { getPeerDependencyFromEditor, signal, effect, namedSignals } from '@lexical/extension';
import { sel, ImportTextFormat, ImportTextStyle, contextValue, $propagateTextAlignToBlockChildren, CoreImportExtension, DOMImportExtension } from '@lexical/html';
...
const INSERT_TABLE_COMMAND = /* @__PURE__ */createCommand('INSERT_TABLE_COMMAND');
...
function $unmergeCellNode(cellNode) {
  ...
  const $createSplitCell = headerState => $copyNode(cell).setColSpan(1).setRowSpan(1).setHeaderStyles(headerState).setWidth(undefined).append($createParagraphNode());
  if (colSpan > 1) {
...
class TableObserver {
  ...
  $getAnchorTableCell() {
    const node = this.anchorCellNodeKey ? $getNodeByKey(this.anchorCellNodeKey) : null;
  ...
  $getFocusTableCell() {
    const node = this.focusCellNodeKey ? $getNodeByKey(this.focusCellNodeKey) : null;
    return $isTableCellNode(node) ? node : null;
  }
  ...
  $clearText() {
    ...
    selectedNodes.forEach(cellNode => {
      if ($isElementNode(cellNode)) {
        ...
        const paragraphNode = $isParagraphNode(firstChild) ? $copyNode(firstChild) : $createParagraphNode();
...
function registerTableWindowHandlers(editor, tableObservers) {
  ...
  return editor.registerRootListener(rootElement => {
    ...
    return registerEventListener(editorWindow, 'pointerdown', pointerDownCallback);
  });
}
function $handleTableClick(editor, event, selectedDOMCell, tableElement, tableObserver, tableObservers) {
  ...
  const createPointerHandlers = startingCell => {
    ...
    removeGestureListeners = registerEventListeners(editorWindow, {
  ...
  const prevAnchorNestedCell = $isRangeSelection(prevSelection) || $isTableSelection(prevSelection) ? $findCellNode(prevSelection.anchor.getNode()) : null;
  const isShiftClickIntoNestedTable = prevAnchorNestedCell !== null && prevAnchorNestedCell.isParentOf(tableNode);
  if (IS_FIREFOX && event.shiftKey && ($isRangeSelection(prevSelection) || $isTableSelection(prevSelection)) && ($isSelectionInTable(prevSelection, tableNode) || isShiftClickIntoNestedTable)) {
...
function applyTableHandlers(tableNode, element, editor, hasTabHandler, tableObservers) {
  ...
  tableObserver.listenersToRemove.add(registerEventListener(tableElement, 'mousedown', onTripleClick, tableObserver.listenerOptions));
  ...
  tableObserver.listenersToRemove.add(registerEventListener(doc, 'paste', event => {
...
function $fixRangeSelectionForSelectedTable(selection, tableObservers) {
  ...
  if (shouldMoveFocus) {
    ...
    // table selection (#8538).
    if (tableObserver.pointerType === 'touch' && tableObserver.isSelecting && tableObserver.isPointerDrag && selection.isCollapsed() && $isRangeSelection(prevSelection) && prevSelection.isCollapsed()) {
...
function $fixTableSelectionForSelectedTable(editor, selection) {
  ...
  const points = domSelection && getDOMSelectionPoints(domSelection, editor.getRootElement());
...
function $isSelectionBeforeTable(selection, tableNode) {
  ...
  return $getCommonAncestor(focusCaret.origin, tableCaret.origin) !== null && $comparePointCaretNext(focusCaret, tableCaret) < 0;
}
  ...
  if ($isRangeSelection(selection)) {
    ...
    if (selection.isCollapsed()) {
      ...
      if (anchor.type === 'element') {
        ...
        const range = getDOMSelectionRange(domSelection, editor.getRootElement());
        if (range === null) {
          return false;
        }
...
function $getTableEdgeCursorPosition(editor, selection, tableNode) {
  ...
  const domAnchorNode = getDOMSelectionPoints(domSelection, editor.getRootElement()).anchorNode;
...
class TableNode extends ElementNode {
  ...
  exportDOM(editor) {
    ...
    const exportedElement = !isHTMLTableElement(element) && isHTMLElement(element) ? element.querySelector('table') : element;
    ...
    if (isHTMLElement(element) && isHTMLTableElement(exportedElement) && exportedElement !== element) {
      if (element.dir) {
...
const TableRule = {
  $import: (ctx, el) => {
    ...
    return [node.splice(0, 0, $descendantsMatching(ctx.$importChildren(el), $isTableRowNode))];
  },
  match: /* @__PURE__ */sel.tag('table'),
  name: '@lexical/table/table'
};
const TableRowRule = {
  $import: (ctx, el) => {
    const height = PIXEL_VALUE_REG_EXP.test(el.style.height) ? parseFloat(el.style.height) : undefined;
    return [$createTableRowNode(height).splice(0, 0, $descendantsMatching(ctx.$importChildren(el), $isTableCellNode))];
  },
  match: /* @__PURE__ */sel.tag('tr'),
  ...
  $import: (ctx, el) => {
    ...
    if (isHeader) {
      ...
      if (scope === 'col') {
        ...
        const isInHeaderRow = isHTMLTableRowElement(parentRow) && (parentRow.parentElement && parentRow.parentElement.nodeName === 'THEAD' || parentRow.rowIndex === 0);
...
const TableExtension = {
  ...
  CoreImportExtension, ([DOMImportExtension, {
