...
function useYjsCollaborationV2__EXPERIMENTAL(editor, id, doc, provider, docMap, name, color, options = {}) {
  ...
  const [binding] = React.useState(() => yjs.createBindingV2__EXPERIMENTAL(editor, id, doc, docMap, {
    excludedProperties,
    getXmlElement,
    rootName
  }));
  ...
  React.useEffect(() => {
    ...
    const removeListener = editor.registerUpdateListener(({
      ...
      if (!tags.has(lexical.SKIP_COLLAB_TAG)) {
        yjs.syncLexicalUpdateToYjsV2__EXPERIMENTAL(binding, provider, prevEditorState, editorState, dirtyElements, dirtyLeaves, normalizedNodes, tags);
      }
    });
    return () => {
...
function CollaborationPlugin({
  ...
  const providerInputs = React.useRef(null);
