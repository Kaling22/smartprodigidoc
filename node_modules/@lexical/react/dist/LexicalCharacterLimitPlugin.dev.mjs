...
import { mergeRegister, DELETE_CHARACTER_COMMAND, $getSelection, $isRangeSelection, $isElementNode, COMMAND_PRIORITY_LOW, HISTORY_MERGE_TAG, $isLeafNode, $getSlotHost, $findMatchingParent, $isTextNode, $setSelection } from 'lexical';
...
function useCharacterLimit(editor, maxCharacters, optional = Object.freeze({})) {
  ...
  useEffect(() => {
    ...
    let lastComputedTextLength = null;
...
function $wrapOverflowedNodes(offset) {
  ...
  for (let i = 0; i < dfsNodesLength; i += 1) {
    ...
    const needsOverflowParent = $isLeafNode(node) && !isSlotValueLeaf && !$findMatchingParent(node, $isOverflowNode);
