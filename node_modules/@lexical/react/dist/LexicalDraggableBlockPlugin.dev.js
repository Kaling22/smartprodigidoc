...
function restoreEditorFocus(editor, rootElement) {
  rootElement.focus({
    preventScroll: true
  });
  editor.update(() => {
...
function useDraggableBlockMenu(editor, anchorElem, menuRef, targetLineRef, isEditable, menuComponent, targetLineComponent, isOnMenu, onElementChanged) {
  const scrollerElem = lexical.getParentElement(anchorElem);
  ...
  react.useEffect(() => {
    ...
    return lexical.mergeRegister(editor.registerCommand(lexical.DRAGOVER_COMMAND, event => {
  ...
  react.useEffect(() => {
    ...
    return lexical.mergeRegister(editor.registerRootListener(rootElement => {
      function onBlur(event) {
        const relatedTarget = event.relatedTarget;
        if (lexical.isHTMLElement(relatedTarget) && isOnMenu(relatedTarget)) {
    ...
    editor.registerCommand(lexical.BLUR_COMMAND, () => {
      ...
      const activeElement = lexical.getActiveElementDeep(lexical.getRootOwnerDocument(rootElement));
      if (rootElement && lexical.isHTMLElement(activeElement) && isOnMenu(activeElement)) {
  ...
  function onDragStart(event) {
    ...
    if (lexical.IS_FIREFOX) {
      ...
      if (rootElement !== null && lexical.getActiveElement(rootElement) !== rootElement) {
...
function DraggableBlockPlugin_EXPERIMENTAL({
  ...
  return useDraggableBlockMenu(editor, anchorElem, menuRef, targetLineRef, isEditable, menuComponent, targetLineComponent, isOnMenu, onElementChanged);
}
