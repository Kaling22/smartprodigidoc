...
function useLexicalIsTextContentEmpty(editor, trim) {
  const [isEmpty, setIsEmpty] = react.useState(() => editor.read('latest', text.$isRootTextContentEmptyCurry(editor.isComposing(), trim)));
  useLayoutEffectImpl(() => {
    function resetIsEmpty() {
      setIsEmpty(editor.read('latest', text.$isRootTextContentEmptyCurry(editor.isComposing(), trim)));
    }
