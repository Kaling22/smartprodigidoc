...
function useRootElement(editor) {
  const [subscribe, getSnapshot] = react.useMemo(() => [editor.registerRootListener.bind(editor), editor.getRootElement.bind(editor)], [editor]);
...
function canShowPlaceholderFromCurrentEditorState(editor) {
  const currentCanShowPlaceholder = editor.read('latest', text.$canShowPlaceholderCurry(editor.isComposing()));
...
function useRichTextSetup(editor) {
  useLayoutEffectImpl(() => {
    return lexical.mergeRegister(richText.registerRichText(editor), dragon.registerDragonSupport(editor));
  }, [editor]);
}
