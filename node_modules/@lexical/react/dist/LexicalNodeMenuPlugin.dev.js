...
function useDynamicPositioning(resolution, targetElement, onReposition, onVisibilityChange) {
  ...
  react.useEffect(() => {
    if (targetElement != null && resolution != null) {
      ...
      const rootScrollParent = rootElement != null ? utils.getScrollParent(rootElement, false) :
      // eslint-disable-next-line no-restricted-syntax
      document.body;
      let ticking = false;
      ...
      const enclosingShadowRoots = lexical.getDOMShadowRoots(rootElement ?? targetElement);
      resizeObserver.observe(targetElement);
      return lexical.mergeRegister(lexical.registerEventListener(window, 'resize', onReposition), lexical.registerEventListener(document, 'scroll', handleScroll, {
        capture: true,
        passive: true
      }), ...enclosingShadowRoots.map(root => lexical.registerEventListener(root, 'scroll', handleScroll, {
        capture: true,
        passive: true
...
function LexicalMenu({
  ...
  react.useEffect(() => {
    return lexical.mergeRegister(editor.registerCommand(lexical.KEY_ARROW_DOWN_COMMAND, payload => {
      const event = payload;
      if (rendersNothing) {
...
function getContainingBlockOrigin(parent) {
  ...
  const start = lexical.isDOMShadowRoot(parent) ? parent.host : parent;
  for (let element = lexical.isHTMLElement(start) ? start : null; element !== null; element = lexical.getParentElement(element)) {
...
function resolveMenuParent(editor) {
  if (!lexical.CAN_USE_DOM) {
    return undefined;
  }
  const rootElement = editor.getRootElement();
  if (rootElement !== null) {
    const root = rootElement.getRootNode();
    if (lexical.isDOMShadowRoot(root)) {
      return root;
    }
...
function useMenuAnchorRef(resolution, setResolution, className, parent, shouldIncludePageYOffset__EXPERIMENTAL = true) {
  ...
  const initialAnchorElement = lexical.CAN_USE_DOM ? lexical.getRootOwnerDocument(editor.getRootElement()).createElement('div') : null;
    ...
    if (nodeKey) {
      editor.update(() => {
        ...
        if (node != null && domElement != null) {
          if (resolution == null) {
            react.startTransition(() => openNodeMenu({
