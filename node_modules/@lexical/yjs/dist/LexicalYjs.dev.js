...
class CollabDecoratorNode {
  ...
  getOffset() {
    ...
    if (!!(this._parent instanceof CollabDecoratorNode)) {
      formatDevErrorMessage(`getOffset: expected parent to be a collab element node`);
    }
...
class CollabElementNode {
  ...
  getOffset() {
    ...
    if (!(collabElementNode instanceof CollabElementNode)) {
...
function createYjsBinding({
  ...
  const rootXmlText = getXmlText ? getXmlText(doc) : doc.get(rootName, yjs.XmlText);
  if (!(rootXmlText instanceof yjs.XmlText && rootXmlText.doc === doc)) {
    formatDevErrorMessage(`createYjsBinding: getXmlText must return an XmlText that is already integrated into the given doc`);
  }
...
function createBindingV2__EXPERIMENTAL(editor, id, doc, docMap, options = {}) {
  ...
  const root = getXmlElement ? getXmlElement(doc) : doc.get(rootName, yjs.XmlElement);
  if (!(root instanceof yjs.XmlElement && root.doc === doc)) {
    formatDevErrorMessage(`createBindingV2: getXmlElement must return an XmlElement that is already integrated into the given doc`);
  }
  if (!(root.nodeName === ROOT_NODE_NAME)) {
    formatDevErrorMessage(`createBindingV2: the root XmlElement must be created without a nodeName (its nodeName is ${root.nodeName}, expected ${ROOT_NODE_NAME})`);
  }
  return {
    ...createBaseBinding(editor, id, doc, docMap, excludedProperties),
 ...
 */
const ROOT_NODE_NAME = 'UNDEFINED';
const baseExcludedProperties = new Set(['__key', '__parent', '__next', '__prev', '__state', '__slotHost',
...
const rootExcludedProperties = new Set(['__cachedText', '__textFormat', '__textStyle']);
...
function $syncSlotContentFromLexical(binding, slotCollab, slotNode, prevNodeMap, dirtyElements, dirtyLeaves) {
  if (slotCollab instanceof CollabElementNode && lexical.$isElementNode(slotNode)) {
  ...
  } else if (slotCollab instanceof CollabTextNode && lexical.$isTextNode(slotNode)) {
    slotCollab.syncPropertiesAndTextFromLexical(binding, slotNode, prevNodeMap);
  } else if (slotCollab instanceof CollabDecoratorNode && lexical.$isDecoratorNode(slotNode)) {
    slotCollab.syncPropertiesFromLexical(binding, slotNode, prevNodeMap);
    slotCollab.syncSlotsFromLexical(binding, slotNode, prevNodeMap, dirtyElements, dirtyLeaves);
  }
}
...
function $seedHostSlots(binding, lexicalNode, hostCollab, sharedType) {
  const slotNames = lexical.$getSlotNames(lexicalNode);
  ...
  const declaresSlots = lexical.getDeclaredSlots(lexicalNode.constructor).length > 0;
  if (slotNames.length === 0 && !declaresSlots) {
    return;
  }
  const slotsY = new yjs.Map();
  let hasSlot = false;
  for (const name of slotNames) {
    const slotNode = lexical.$getSlot(lexicalNode, name);
    if (slotNode === null) {
      continue;
    }
...
function $createCollabNodeFromLexicalNode(binding, lexicalNode,
...
parent) {
  ...
  if (lexical.$isElementNode(lexicalNode)) {
    ...
    if (!(parent instanceof CollabElementNode)) {
      formatDevErrorMessage(`Expected parent of a text node to be a collab element node`);
    ...
    if (!(parent instanceof CollabElementNode)) {
      formatDevErrorMessage(`Expected parent of a linebreak node to be a collab element node`);
    }
    const map = new yjs.Map();
...
function $getOrInitCollabNodeFromSharedType(binding, sharedType, parent) {
  ...
  if (collabNode === undefined) {
    ...
    if (!(targetParent instanceof CollabElementNode || targetParent instanceof CollabDecoratorNode && decoratorHostsSlotSharedType(targetParent, sharedType))) {
      formatDevErrorMessage(`Expected parent to be a collab element node, or a collab decorator node hosting this shared type as a named slot`);
    }
    if (sharedType instanceof yjs.XmlText) {
      ...
      if (!(targetParent instanceof CollabElementNode)) {
        formatDevErrorMessage(`Expected parent of a text or linebreak node to be a collab element node`);
      }
      if (type === 'linebreak') {
...
function createLexicalNodeFromCollabNode(binding, collabNode, parentKey) {
  ...
  if (collabNode instanceof CollabElementNode) {
    ...
    collabNode.syncPropertiesFromYjs(binding, null);
    if (!lexical.$isDecoratorNode(lexicalNode)) {
      formatDevErrorMessage(`Expected a decorator node for a collab decorator node`);
    }
  ...
  for (let i = 0; i < properties.length; i++) {
    ...
    if (prevValue !== nextValue) {
      if (nextValue instanceof yjs.Doc) {
        ...
        if (lexical.isLexicalEditor(prevValue)) {
          prevValue._key = key;
          nextValue = prevValue;
        } else {
...
function $syncSlotsFromYjsShared(binding, slotsParent, lexicalNode, ownerCollab) {
  ...
  const yNames = slotsY instanceof yjs.Map ? new Set(slotsY.keys()) : new Set();
  for (const name of lexical.$getSlotNames(lexicalNode)) {
    if (!yNames.has(name)) {
      ...
      lexical.$removeSlot(lexicalNode, name);
    }
  }
  if (!(slotsY instanceof yjs.Map)) {
    return;
  ...
  for (const [name, slotSharedType] of slotsY.entries()) {
    ...
    if (!(slotSharedType instanceof yjs.XmlText || slotSharedType instanceof yjs.XmlElement || slotSharedType instanceof yjs.Map)) {
      continue;
    }
    const existingSlot = lexical.$getSlot(lexicalNode, name);
    const existingCollab = existingSlot === null ? undefined : binding.collabNodeMap.get(existingSlot.__key);
    if (existingCollab !== undefined && existingCollab.getSharedType() === slotSharedType) {
    ...
    if (slotSharedType._collabNode === undefined && typeof sharedTypeGet(slotSharedType, '__type') !== 'string') {
      continue;
    }
    ...
    if (slotSharedType instanceof yjs.Map && ownerCollab instanceof CollabDecoratorNode) {
      continue;
    }
    ...
    if (cachedCollab !== undefined) {
      const cachedNode = lexical.$getNodeByKey(cachedCollab._key);
      if (cachedNode !== null && (cachedNode.isAttached() || (lexical.$isElementNode(cachedNode) || lexical.$isDecoratorNode(cachedNode)) && cachedNode.getLatest().__slotHost !== null)) {
        continue;
      }
    }
    ...
    lexical.$setSlot(lexicalNode, name, slotLexicalNode);
  }
}
...
function $syncSlotsFromLexicalShared(binding, slotsParent, nextLexicalNode, prevNodeMap, dirtyElements, dirtyLeaves, ownerCollab) {
  const slotNames = lexical.$getSlotNames(nextLexicalNode);
  const existing = slotsParent.getAttribute(SLOTS_ATTR_KEY);
  if (slotNames.length === 0 && !(existing instanceof yjs.Map)) {
    return;
  }
  let slotsY;
  ...
  const collabNodeMap = binding.collabNodeMap;
  for (const name of slotNames) {
    const slotNode = lexical.$getSlot(nextLexicalNode, name);
    if (slotNode === null) {
      continue;
    }
    const slotCollab = collabNodeMap.get(slotNode.__key);
    if (slotCollab !== undefined && slotsY.get(name) === slotCollab.getSharedType()) {
      $syncSlotContentFromLexical(binding, slotCollab, slotNode, prevNodeMap, dirtyElements, dirtyLeaves);
    } else {
...
const isRootElement = el => el.nodeName === ROOT_NODE_NAME;
const $createOrUpdateNodeFromYElement = (el, binding, keysChanged, childListChanged, snapshot, prevSnapshot, computeYChange) => {
  ...
  for (const k in attrs) {
    ...
    } else if (k !== SLOTS_ATTR_KEY && k !== '__proto__' && k !== 'constructor' && k !== 'prototype') {
  ...
  if (node instanceof lexical.ElementNode || lexical.$isDecoratorNode(node)) {
    ...
    if (slotsY instanceof yjs.Map) {
      ...
      const slotEntries = snapshot === undefined ? Array.from(slotsY.entries()) : Object.entries(yjs.typeMapGetAllSnapshot(slotsY, snapshot));
      for (const [name, slotType] of slotEntries) {
        ...
        let slotNode = null;
        if (slotType instanceof yjs.XmlElement) {
          slotNode = $createOrUpdateNodeFromYElement(slotType, binding, new Set(), false, snapshot, prevSnapshot, computeYChange);
        }
        if (slotNode === null) {
          continue;
        }
        ...
        const existingSlot = lexical.$getSlot(node, name);
        if (existingSlot !== null && existingSlot.getKey() === slotNode.getKey()) {
          continue;
        }
        if (!$isSlotValueNode(slotNode)) {
          ...
          if (!slotNode.isAttached()) {
            $deleteMappingForSubtree(slotType, binding);
          }
          continue;
        }
        ...
        lexical.$setSlot(node, name, slotNode);
      }
    }
...
const $createSlotsYType = (node, binding) => {
  const names = lexical.$getSlotNames(node);
  if (names.length === 0 && lexical.getDeclaredSlots(node.constructor).length === 0) {
    return undefined;
  }
  const slotsY = new yjs.Map();
  for (const name of names) {
    const slotNode = lexical.$getSlot(node, name);
    if (slotNode == null) {
      continue;
    }
...
const $updateSlotsYType = (yDomFragment, node, binding, dirtyElements, y) => {
  const names = lexical.$getSlotNames(node);
  ...
  if (names.length === 0 && !(existing instanceof yjs.Map) && lexical.getDeclaredSlots(node.constructor).length === 0) {
    return;
  }
  let slotsY;
  if (existing instanceof yjs.Map) {
  ...
  for (const name of names) {
    ...
    if (slotY instanceof yjs.XmlElement && (slotNode instanceof lexical.ElementNode || lexical.$isDecoratorNode(slotNode)) && slotY === binding.mapping.getSharedType(slotNode)) {
      if (dirtyElements.has(slotNode.getKey())) {
        $updateYFragment(y, slotY, slotNode, binding, dirtyElements);
      }
    } else {
      if (slotY instanceof yjs.XmlElement) {
...
const $createSlotValueType = (slotNode, binding) => {
  const type = $createTypeFromElementNode(slotNode, binding);
  if (lexical.$isDecoratorNode(slotNode) && binding.mapping.getSharedType(slotNode) === undefined) {
...
// not correctness.
const $equalSlots = (ytype, lnode, binding) => {
  const names = lexical.$getSlotNames(lnode);
  ...
  for (const name of names) {
    const slotNode = lexical.$getSlot(lnode, name);
    const slotY = slotsY.get(name);
    if (slotNode === null || !(slotY instanceof yjs.XmlElement) || binding.mapping.get(slotY) !== slotNode) {
      return false;
    }
  }
  return true;
...
const $updateYFragment = (y, yDomFragment, node, binding, dirtyElements) => {
  ...
  for (; left < minCnt; left++) {
    ...
    if (leftY instanceof yjs.XmlHook) {
      ...
      if (!(leftL instanceof Array) && (leftL instanceof lexical.ElementNode || lexical.$isDecoratorNode(leftL)) && dirtyElements.has(leftL.getKey())) {
  ...
  for (; right + left < minCnt; right++) {
    ...
    if (rightY instanceof yjs.XmlHook) {
      ...
      if (!(rightL instanceof Array) && (rightL instanceof lexical.ElementNode || lexical.$isDecoratorNode(rightL)) && dirtyElements.has(rightL.getKey())) {
...
const ychangeState = /* @__PURE__ */lexical.createState(STATE_KEY, {
  isEqual: (a, b) => a === b,
...
const SUPPORTS_CSS_HIGHLIGHTS = typeof Highlight !== 'undefined' && typeof CSS !== 'undefined' && 'highlights' in CSS;
...
function getCursorHighlightSheet(binding) {
  ...
  const ownerDocument = lexical.getRootOwnerDocument(rootElement);
  ...
  const target = lexical.isDOMShadowRoot(root) ? root : ownerDocument;
...
function createRelativePosition(point, binding, assoc = 0) {
  ...
  return yjs.createRelativePositionFromTypeIndex(sharedType, offset, assoc);
}
function createRelativePositionV2(point, binding, assoc = 0) {
  ...
  if (point.type === 'text') {
    ...
    return yjs.createRelativePositionFromTypeIndex(yType, adjustedOffset, assoc);
    ...
    return yjs.createRelativePositionFromTypeIndex(yType, yIndex, assoc);
  }
  return null;
}
...
function createCursorSelection(cursor, binding, clientID, anchorKey, anchorOffset, focusKey, focusOffset, selectionHighlight, theme = {}) {
  ...
  const ownerDocument = lexical.getRootOwnerDocument(binding.editor.getRootElement());
...
function updateCursor(binding, cursor, nextSelection, nodeMap, theme = {}) {
  ...
  if (highlight !== null) {
    ...
    if (caretRect.height === 0 && lexical.$isLineBreakNode(focusNode)) {
...
function getAnchorAndFocusCollabNodesForUserState(binding, userState) {
  ...
  if (anchorPos !== null && focusPos !== null) {
    ...
    if (anchorAbsPos !== null && focusAbsPos !== null) {
      [anchorCollabNode, anchorOffset] = getCollabNodeAndOffset(binding, anchorAbsPos.type, anchorAbsPos.index);
      [focusCollabNode, focusOffset] = getCollabNodeAndOffset(binding, focusAbsPos.type, focusAbsPos.index);
    }
  }
  return {
    anchorCollabNode,
 ...
 */
function isSharedTypeInBinding(binding,
// eslint-disable-next-line @typescript-eslint/no-explicit-any
sharedType) {
...
function getCollabNodeAndOffset(binding,
// eslint-disable-next-line @typescript-eslint/no-explicit-any
sharedType, offset) {
  const collabNode = sharedType._collabNode;
    ...
    if (clientID !== 0 && clientID !== localClientID) {
      ...
      if (focusing) {
        ...
        if (anchorKey !== null && focusKey !== null) {
          ...
          if (selection === null) {
            selection = createCursorSelection(cursor, binding, clientID, anchorKey, anchorOffset, focusKey, focusOffset, selectionHighlight, collabTheme);
          } else {
            const anchor = selection.anchor;
            const focus = selection.focus;
...
function $syncEvent(binding, event) {
  ...
  if (event instanceof yjs.YMapEvent && target instanceof yjs.Map && target._item != null && target._item.parentSub === SLOTS_ATTR_KEY && (target.parent instanceof yjs.XmlText || target.parent instanceof yjs.XmlElement)) {
    ...
    yjs.iterateDeletedStructs(transaction, transaction.deleteSet, struct => {
      if (struct.constructor === yjs.Item) {
        ...
        if (type) {
          ...
          if (collabNode !== undefined && binding.collabNodeMap.get(collabNode._key) === collabNode) {
  ...
  syncWithTransaction(binding, () => {
    currEditorState.read(() => {
      ...
      if (dirtyElements.has('root')) {
        ...
        if (nextLexicalRoot.getChildrenSize() === 0 && prevEditorState._nodeMap.size > 1) {
...
function $syncEventV2(binding, event) {
  ...
  if (target instanceof yjs.XmlElement && event instanceof yjs.YXmlEvent) {
  ...
  } else if (target instanceof yjs.Map && event instanceof yjs.YMapEvent && target._item != null && target._item.parentSub === SLOTS_ATTR_KEY) {
    ...
    const parent = target.parent;
    if (parent instanceof yjs.XmlElement) {
      $createOrUpdateNodeFromYElement(parent, binding, new Set([SLOTS_ATTR_KEY]), false);
    } else {
      {
        formatDevErrorMessage(`Expected XmlElement parent for slots Y.Map`);
      }
    }
  } else {
    {
  ...
  syncWithTransaction(binding, () => {
    currEditorState.read(() => {
      if (dirtyElements.has('root')) {
        ...
        $updateYFragment(binding.doc, binding.root, nextLexicalRoot, binding, new Set([...dirtyElements.keys(), ...dirtyLeaves]));
        // If a local edit emptied the root, schedule recovery outside the
        // collaboration/historic tag so the paragraph syncs back to Yjs.
        // Mirrors the $ensureEditorNotEmpty call in syncYjsChangesToLexicalV2's onUpdate.
        // Only trigger when the previous state had content (prevNodeMap.size > 1 means
        // there were nodes beyond root), so we don't fire on initial empty-editor updates.
        if (!isFromYjs && nextLexicalRoot.getChildrenSize() === 0 && prevEditorState._nodeMap.size > 1) {
          binding.editor.update(() => $ensureEditorNotEmpty());
        }
      }
      const selection = lexical.$getSelection();
...
const CONNECTED_COMMAND = /* @__PURE__ */lexical.createCommand('CONNECTED_COMMAND');
const TOGGLE_CONNECT_COMMAND = /* @__PURE__ */lexical.createCommand('TOGGLE_CONNECT_COMMAND');
const DIFF_VERSIONS_COMMAND__EXPERIMENTAL = /* @__PURE__ */lexical.createCommand('DIFF_VERSIONS_COMMAND');
const CLEAR_DIFF_VERSIONS_COMMAND__EXPERIMENTAL = /* @__PURE__ */lexical.createCommand('CLEAR_DIFF_VERSIONS_COMMAND');
...
exports.$getAnchorAndFocusForUserState = $getAnchorAndFocusForUserState;
...
exports.createYjsBinding = createYjsBinding;
...
exports.removeCursorHighlightRule = removeCursorHighlightRule;
