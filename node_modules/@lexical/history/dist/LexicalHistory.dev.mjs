...
import { effect, signal, namedSignals, batch, getPeerDependencyFromEditor } from '@lexical/extension';
import { mergeRegister, UNDO_COMMAND, COMMAND_PRIORITY_EDITOR, REDO_COMMAND, CLEAR_EDITOR_COMMAND, CLEAR_HISTORY_COMMAND, CAN_REDO_COMMAND, CAN_UNDO_COMMAND, COMPOSITION_START_TAG, HISTORIC_TAG, COMPOSITION_END_TAG, PASTE_TAG, CUT_TAG, HISTORY_PUSH_TAG, HISTORY_MERGE_TAG, $isRangeSelection, $isTextNode, $isRootNode } from 'lexical';
...
function createMergeActionGetter(editor, delayOrStore, dateNow) {
  ...
  return (prevEditorState, nextEditorState, currentHistoryEntry, dirtyLeaves, dirtyElements, tags) => {
    ...
    // handler produced it.
    const changeType = tags.has(PASTE_TAG) || tags.has(CUT_TAG) ? OTHER : getChangeType(prevEditorState, nextEditorState, dirtyLeaves, dirtyElements, editor.isComposing());
    const mergeAction = (() => {
...
const HistoryExtension = {
  ...
  register: (editor, config, state) => {
    ...
    const syncFromHistoryState = historyState => batch(() => {
...
function hmrRestoreCount(editor) {
  const peer = editor ? getPeerDependencyFromEditor(editor, HMR_EXTENSION_NAME) : undefined;
