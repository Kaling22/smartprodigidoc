...
function createMergeActionGetter(editor, delayOrStore, dateNow) {
  ...
  return (prevEditorState, nextEditorState, currentHistoryEntry, dirtyLeaves, dirtyElements, tags) => {
    ...
    // handler produced it.
    const changeType = tags.has(lexical.PASTE_TAG) || tags.has(lexical.CUT_TAG) ? OTHER : getChangeType(prevEditorState, nextEditorState, dirtyLeaves, dirtyElements, editor.isComposing());
    const mergeAction = (() => {
...
const HistoryExtension = {
  ...
  init: () => ({
    canRedo: extension.signal(false),
    canUndo: extension.signal(false)
  }),
  ...
  register: (editor, config, state) => {
    ...
    const syncFromHistoryState = historyState => extension.batch(() => {
...
function hmrRestoreCount(editor) {
  const peer = editor ? extension.getPeerDependencyFromEditor(editor, HMR_EXTENSION_NAME) : undefined;
