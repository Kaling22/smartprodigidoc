 ...
 */
const codeMetaState = /* @__PURE__ */lexical.createState('mdCodeMeta', {
...
function $isUnreplaceableBlock(parentNode) {
  return richText.$isQuoteNode(parentNode);
}
...
function $setCodeMeta(parentNode, meta) {
  ...
  if (meta && codeCore.$isCodeNode(codeNode)) {
    lexical.$setState(codeNode, codeMetaState, meta);
  }
}
...
const CODE = {
  ...
  export: node => {
    ...
    const meta = language ? lexical.$getState(node, codeMetaState) : '';
...
function createSelectionMarkdownExport(transformers, shouldPreserveNewLines = false) {
  ...
  return selection => {
    ...
    for (const child of lexical.$getRoot().getChildren()) {
      const {
        shouldInclude,
        markdown
    ...
    lexical.$isParagraphNode(previousNode) || richText.$isQuoteNode(previousNode) || list.$isListNode(previousNode))) {
      ...
      if (list.$isListNode(previousNode)) {
        ...
        if (lastDescendant == null) {
          ...
          targetNode = lexical.$findMatchingParent(lastDescendant, list.$isListItemNode);
        }
      }
...
function $runTextFormatTransformers(anchorNode, anchorOffset, textFormatTransformers) {
  ...
  for (const matcher of matchers) {
    ...
    for (const format of matcher.format) {
      nextSelection.formatText(format, lexical.TEXT_TYPE_TO_FORMAT[format]);
    }
...
function registerMarkdownShortcuts(editor, transformers = TRANSFORMERS) {
  ...
  return lexical.mergeRegister(editor.registerUpdateListener(({
    ...
    if (!lexical.$isRangeSelection(prevSelection) || !lexical.$isRangeSelection(selection) || !selection.isCollapsed() || selection.is(prevSelection) && !isCompositionEnd) {
      return;
    }
...
function $generateNodesFromMarkdownString(markdown, transformers = TRANSFORMERS, shouldPreserveNewLines = false, shouldMergeAdjacentLines = false) {
  const sanitizedMarkdown = shouldPreserveNewLines ? markdown : normalizeMarkdown(markdown, shouldMergeAdjacentLines);
  const container = new lexical.ArtificialNode__DO_NOT_USE();
...
exports.isTableRowDivider = isTableRowDivider;
